...
// resolvedModel holds pre-computed model data shared by both generators.
...
// resolveAllModels iterates the model config, looks up API data, and returns
// a deterministically-ordered slice of resolved models.
func resolveAllModels(apiModels map[string]OpenRouterModel) []resolvedModel {
	...
	out := make([]resolvedModel, 0, len(modelIDs))
	for _, id := range modelIDs {
		...
		out = append(out, resolvedModel{
			...
			API:         resolveModelAPIForManifest(id, "openrouter"),
			Caps:        detectCapabilities(id, apiModel, hasAPIData),
		})
	}
	return out
}
...
func generateGoFile(apiModels map[string]OpenRouterModel, outputPath string) error {
	models := resolveAllModels(apiModels)
...
`,
			...
			m.Caps.ContextWindow, m.Caps.MaxOutputTokens, availableToolsGo(m.Caps),
		))
	}

...
if echo "$contracts_imports" | rg -q '^github.com/beeper/ai-bridge/modules/runtime'; then
  fail "modules/contracts must not import feature/runtime modules"
fi

for imports in "$runtime_imports" "$aiproxy_imports"; do
...
if echo "$ai_bridge_deps" | rg -q 'github.com/beeper/ai-bridge/pkg/simpleruntime'; then
  fail "ai bridge must not depend on retired pkg/simpleruntime/*"
fi
...
# Retired runtime path must not be used anywhere.
if rg -q 'github.com/beeper/ai-bridge/pkg/simpleruntime' pkg modules bridges; then
  fail "retired pkg/simpleruntime imports are not allowed"
fi

# Shared pkg/* packages must not import dedicated repos.

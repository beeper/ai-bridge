// Package pruning implements context window pruning for OpenAI-style
// chat prompts. It provides configurable soft-trim and hard-clear phases
// to keep conversation history within token budgets.
package pruning
...
// Config configures context pruning behavior.
type Config struct {
...
// DefaultConfig returns sensible default pruning settings.
...
// EstimateMessageChars estimates character count from a message.
...
// MakeToolPrunablePredicate creates a function that checks if a tool is prunable.
func MakeToolPrunablePredicate(config *Config) func(toolName string) bool {
	if config == nil {
...
// softTrimToolResult truncates a large tool result keeping head and tail
func softTrimToolResult(content string, config *Config) string {
...
// findAssistantCutoffIndex finds the index where protected tail starts.
...
// PruneContext prunes messages to fit within context window.
...
func PruneContext(
	...
	// Convert token window to char window
...
// ApplyDefaults fills in default values for any missing config fields.
...
// Uses aggressive settings for emergency reduction.

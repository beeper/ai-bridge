...
func buildSyntheticGeneratedImagesMessage(imageParts []openai.ChatCompletionContentPartUnionParam) openai.ChatCompletionMessageParamUnion {
	parts := make([]openai.ChatCompletionContentPartUnionParam, 0, 1+len(imageParts))
	parts = append(parts, openai.ChatCompletionContentPartUnionParam{
		OfText: &openai.ChatCompletionContentPartTextParam{Text: "[Previously generated image(s) for reference]"},
	})
	parts = append(parts, imageParts...)
	return openai.ChatCompletionMessageParamUnion{
		OfUser: &openai.ChatCompletionUserMessageParam{

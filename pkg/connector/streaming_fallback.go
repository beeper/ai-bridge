...
// tryChatCompletionsWithToolFallback tries Chat Completions, and if tools are
// rejected, retries once more with tool-calling disabled.
func (oc *AIClient) tryChatCompletionsWithToolFallback(
	ctx context.Context,
	evt *event.Event,
...
func (oc *AIClient) streamingResponseWithToolSchemaFallback(
	...
	if IsToolUniquenessError(err) {
		...
		return oc.tryChatCompletionsWithToolFallback(ctx, evt, portal, meta, messages)
	}

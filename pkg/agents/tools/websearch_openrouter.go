...
// WebSearchOpenRouter is the OpenRouter web search tool definition.
var WebSearchOpenRouter = &Tool{
	Tool: mcp.Tool{
...
func executeWebSearchOpenRouter(ctx context.Context, args map[string]any) (*Result, error) {
	query, err := ReadString(args, "query", true)
	if err != nil {
		return ErrorResult("web_search_openrouter", err.Error()), nil
	}
	...
	if err != nil {
		return ErrorResult("web_search_openrouter", fmt.Sprintf("search failed: %v", err)), nil
	}
